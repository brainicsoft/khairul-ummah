...
export const refundBkashPayment = async (payload: any) => {
  ...
  try {
    ...
    console.error(
      '[refundBkashPayment] error:',
      error.response?.data || error.message || error,
    );
    throw new CustomError(
      502,
      error.response?.data?.message || error.message || 'bKash refund failed',
    );
  }
};
...
export const listBkashSubscriptions = async (
  page: number,
  size: number,
  headersExtra: any = {},
) => {
  const urlPath = `/api/subscriptions/${page}/${size}`;
  const headers = generateBkashAutopayHeaders(
    'GET',
    urlPath,
    {},
    bkashKey,
  ...
  try {
    const { data } = await axios.get(`${bkashRecurringUrl}${urlPath}`, {
      headers,
    });
    return data;
  } catch (error: any) {
    console.error(
      '[listBkashSubscriptions] error:',
      error.response?.data || error.message || error,
    );
    throw new CustomError(
      502,
      error.response?.data?.message ||
        error.message ||
        'bKash list subscriptions failed',
    );
  }
};

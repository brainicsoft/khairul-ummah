...
export const createPaymentController: RequestHandler = catchAsync(
  async (req, res) => {
...
// autopay controllers moved to `autopay` module

export const verifyBkashController: RequestHandler = catchAsync(
  async (req, res) => {
...
export const sslcommerzSuccessController: RequestHandler = catchAsync(
  async (req, res) => {
    ...
    if (!tran_id) {
      ...
      return res.redirect(
        `${frontendUrl}/payment-status?status=failed&message=Transaction%20ID%20missing`,
      );
    }
    ...
    try {
      ...
      if (result.success) {
        ...
        return res.redirect(
          `${frontendUrl}/payment-status?status=failed&message=${encodeURIComponent(result.message)}`,
        );
      }
    } catch (error) {
      console.error('[v0] SSLCommerz Success Error:', error);
      return res.redirect(
        `${frontendUrl}/payment-status?status=error&message=Payment%20processing%20error`,
      );
    }
  },
);

export const sslcommerzCancelController: RequestHandler = catchAsync(
  async (req, res) => {
    ...
    return res.redirect(
      `${frontendUrl}/payment-status?status=cancelled&message=Payment%20cancelled&tran_id=${tran_id}`,
    );
  },
);

export const sslcommerzIPNController: RequestHandler = catchAsync(
  async (req, res) => {
...
export const getPaymentStatusController: RequestHandler = catchAsync(
  async (req, res) => {
...
export const getAllPaymentController: RequestHandler = catchAsync(
  async (req, res) => {
...
export const getPaymentByIdController: RequestHandler = catchAsync(
  async (req, res) => {
...
export const updatePaymentByIdController: RequestHandler = catchAsync(
  async (req, res) => {
...
export const deletePaymentByIdController: RequestHandler = catchAsync(
  async (req, res) => {
...
// autopay endpoints moved to separate module `autopay`

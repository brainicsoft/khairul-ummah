...
import DonorProfileSetupModal from "@/components/payment/DonorProfileSetupModal"
import { useLazyCheckPhoneQuery } from "@/redux/features/auth/authApi"
import {
  clearPendingDonorProfile,
  getPendingDonorProfile,
...
export default function PaymentStatusPage() {
  ...
  const [pendingDonor, setPendingDonor] = useState<PendingDonorProfile | null>(null)
  const [checkPhone] = useLazyCheckPhoneQuery()

  useEffect(() => {
    if (status !== "success") return

    const pending = getPendingDonorProfile()
    if (!pending?.phone) return

    let cancelled = false

    checkPhone(pending.phone)
      .unwrap()
      .then((result) => {
        if (cancelled) return
